InterceptDiyStr = function (sName, nShowCount) {
    if (sName == "") {
        return sName;
    }
    var showName = ""
...
// 微信截屏分享
var screenShotM = function(picJPGName, type) {
    console.log("screenShotM => picJPGName=" + picJPGName);
    var winSize = cc.director.getWinSize();
    var tex = new cc.RenderTexture(winSize.width, winSize.height,
    ...
    if (cc.sys.os == cc.sys.OS_ANDROID) {
        var result = tex.saveToFile(picJPGName, cc.IMAGE_FORMAT_JPEG, false, function() {
            var jsonTxt = "{\"type\":\"img\",\"scene\":" + type.toString() + ",\"data\":\"" + (imgPath + picJPGName) + "\"}";
            ...
            //jsb.reflection.callStaticMethod( "org/cocos2dx/javascript/AppActivity", "pngShare", "(Ljava/lang/String;)V", picJPGName);
        });
    } else if ( cc.sys.os == cc.sys.OS_IOS ) {
        var result = tex.saveToFile(picJPGName, cc.IMAGE_FORMAT_JPEG, false, function() {
            if (type == 0){
                jsb.reflection.callStaticMethod("JS2OC", "Imageshare2WxSession:", imgPath + picJPGName);
            } else if (type == 1) {
                jsb.reflection.callStaticMethod("JS2OC", "Imageshare2TimeLine:", imgPath + picJPGName);
            }
        });
    }
}
// 分享
var wxWebShareGameRoom = function(gameid, roomid) {
    var shareUrl = "http://downloadjz.kkvs.com/download/private.html?game=" + gameid + "&roomid=" + roomid;
    var title = "荆都麻将";
    var desc = "一款高大上的游戏平台。好玩，刺激!小伙伴们，我在" + roomid +"房等你来战！";
    var shareTxt = "";
    if(cc.sys.os == cc.sys.OS_ANDROID) {
        shareTxt = "{\"type\":\"web\",\"scene\":0,\"data\":\"" + shareUrl + "\",\"title\":\"" + title + "\",\"des\":\"" + desc + "\"}";
    } else if ( cc.sys.os == cc.sys.OS_IOS ) {
        shareTxt = shareUrl + "," + title + "," + desc;
    }
    wxWebShareOnlyM(shareTxt);
}
var wxWebShareOnlyM = function(str) {
    //var desc = "http://kdly.kkvs.cn/html/hubeily_mob.html,无外挂+无漏洞《本地约局 - 口袋癞油》,天气闷热，不想出门！我们还是微信约一手";
    if (!cc.sys.isNative) {
        return;
    }
    if(cc.sys.os == cc.sys.OS_ANDROID) {
        ...
        jsb.reflection.callStaticMethod("JS2OC", "share2WxSession:", str);
    }
}
var wxShareTimeLineM = function(str) {
    //var desc = "http://kdly.kkvs.cn/html/hubeily_mob.html,无外挂+无漏洞《本地约局 - 口袋癞油》,天气闷热，不想出门！我们还是微信约一手";
    if (!cc.sys.isNative) {
        return;
    }
...
var shareCallBack = function (args) {
    ...
    if (sys_name == "android" && platform == "wx") {
        ...
        if (reg_web.test(data)) {
            KKVS.Event.fire("wechat_sdk_share_web");
        }
    } else if(sys_name == "ios" && platform == "wx"){
        KKVS.Event.fire("wechat_sdk_share_web");
    }
};
var httpOnShareWebCBMain = function (times) {
};
shareCallBackSuccessIos = function( platform){
 ...
 */
OxLogin = function (acc, pwd) {
    KKVS.Acc = acc;
    if (pwd !== "") {
        KKVS.Pwd = pwd;
        KKVS.Pwd_MD5 = hex_md5(pwd);
    } else {
        KKVS.Pwd = "As09#^oF";
...
addFavorite2 = function(title, url) {
...
enterRoom = function (field_id, error_str) {
...
enterGame = function (room_id, room_type, min_score, max_score, service_pay) {
    var acc = KKVS.GAME_ACC; //KKVS.Acc;
    var pwd = KKVS.Pwd_MD5; //KKVS.Pwd;
    //if(!acc || acc == "") {
    //    var args = {eventType: 1, msg: "进入游戏失败,帐号不能为空", pro: null, winType: 1};
    //    KKVS.Event.fire("createTips", args);
    //    return;
    //}
    ...
    if(gameEngine.app == undefined) {
        cc.log("->gameEngine.app == undefined");
        var args = new gameEngine.gameEngineArgs();
        //set login ip address
        args.ip = "139.196.173.129"; //"139.196.171.208"; //"61.164.110.67"; //"60.191.221.82"; //
        args.port = 10100;
    ...
    if(!gameEngine.app.socket) {
        ...
        var login_extraDatas = { player_id : KKVS.KBEngineID };
...
putNodeToScreenOnX = function (node, screen_x, dx) {
    var pos = screen_x;
    var parent = node.getParent();
    if (!parent) {
...
putNodeToScreenOnY = function (node, screen_y, dy) {
    var pos = screen_y;
    var parent = node.getParent();
    if (!parent) {
...
toScreenLT = function (node, dp) { //至左上
    var pos = screenLT();
    putNodeToScreen(node, pos, dp);
}
toScreenRT = function (node, dp) { //至右上
    var pos = screenRT();
    putNodeToScreen(node, pos, dp);
}
toScreenLB = function (node, dp) { //至左下
    var pos = screenLB();
    putNodeToScreen(node, pos, dp);
}
toScreenRB = function (node, dp) { //至右下
    var pos = screenRB();
    putNodeToScreen(node, pos, dp);
}
toScreenC = function (node, dp) { //至中
    var pos = screenC();
    putNodeToScreen(node, pos, dp);
}
toScreenLC = function (node, dp) { //至左中
    var pos = screenLC();
    putNodeToScreen(node, pos, dp);
}
toScreenRC = function (node, dp) { //至右中
    var pos = screenRC();
    putNodeToScreen(node, pos, dp);
}
toScreenBC = function (node, dp) { //至下中
    var pos = screenBC();
    putNodeToScreen(node, pos, dp);
}
toScreenTC = function (node, dp) { //至上中
    var pos = screenTC();
    putNodeToScreen(node, pos, dp);
}
toScreenL = function (node, dx) { //至左
    var pos = screenLB();
    putNodeToScreenOnX(node, pos, dx);
}
toScreenR = function (node, dx) { //至右
    var pos = screenRB();
    putNodeToScreenOnX(node, pos, dx);
}
toScreenT = function (node, dy) { //至上
    var pos = screenLT();
    putNodeToScreenOnY(node, pos, dy);
}
toScreenB = function (node, dy) { //至下
    var pos = screenLB();
    putNodeToScreenOnY(node, pos, dy);
}
isVisitorLogin = function () {
...
getFace = function (face_id, gender) {
...
getRedPacketTxt = function (gold) {
    var temp = parseInt(gold) / 100;
    temp = temp.toFixed(2);
    return temp.toString();
}
getGoldTxt = function (gold) {
    //var temp = parseInt(gold) / 100;
    //temp = temp.toFixed(2);
    //return temp.toString();
    return gold.toString();
}
getGoldVal = function (gold) {
    //var temp = parseInt(gold) / 100;
    //temp = temp.toFixed(2);
    //return temp;
    return parseInt(gold);
}
isMoblieNumber = function (str) {
    if (typeof str != 'string') {
        return false;
    }
    return /^1[34578]\d{9}$/.test(str);
}
encryptMoblieNumber = function (str) {
...
isWXNumber = function (str) {
    if (typeof str != 'string') {
        return false;
    }
    return /^[a-zA-Z]{1}[-_a-zA-Z0-9]{5,19}$/.test(str);
}
//install data
...
openInstallCallBack = function(str){

}

openInstallSplit = function(str){
    KKVS.installData = {};
...
var getWakeUpData = function () {
    var data = {};
    var dataStr = "";
    if (cc.sys.isNative) {
        if (cc.sys.os == cc.sys.OS_ANDROID) {
            dataStr = jsb.reflection.callStaticMethod(
                "org/cocos2dx/javascript/AppActivity",
                "scheme",
                "()Ljava/lang/String;");
        } else if (cc.sys.os == cc.sys.OS_IOS) {
            dataStr = jsb.reflection.callStaticMethod("JS2OC", "getscheme");
        }
    }
    if (dataStr.length > 2) {
        ...
        for(var i = 0, l = objArr.length; i < l; ++i) {
            ...
            if (dataStrArray.length == 2) {
                data[dataStrArray[0]] = dataStrArray[1];
            }
        }
    }
    return data;
...
var getOrderNumberForAndroid = function(propID) {
    ...
    var url = "http://clientweb.kkvs.com/MobileApi/PostPayJZ";
    ...
    HttpManager.GetMessage(url, params, METHOD_POST, function(data) {
        ...
        if (m_data != null) {
            ...
            var msg = partnerId + "," + prepayId + "," + nonceStr + "," + timeStamp + "," + sign2 + ",jn8706PyGyK53VcjyqysgFCxgoltqBT2";
            if (cc.sys.os == cc.sys.OS_IOS) {
                jsb.reflection.callStaticMethod("JS2OC", "payWithWeChat:", msg);
            } else if (cc.sys.os == cc.sys.OS_ANDROID) {
...
var emulatorCheck = function () {
    return false;
    var bRet = false;
    if (cc.sys.isNative && cc.sys.os == cc.sys.OS_ANDROID) {
        var emulateValue = jsb.reflection.callStaticMethod(
            "org/cocos2dx/javascript/AppActivity",
            "emulatorCheck",
            "()I");
        bRet = emulateValue == 1 ? true : false;
    }
    return bRet;
}

var purgeCachedDataByHand = function() {
    console.log("purgeCachedDataByHand");
}