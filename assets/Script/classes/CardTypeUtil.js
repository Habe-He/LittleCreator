...
// "单张"
cardTypeUtil.oneCard = 1;

// "对子"
cardTypeUtil.doubleCard = 2;

// "三张不带"
cardTypeUtil.threeCard = 3;

// "炸弹（四张）"(包括5到8炸)
cardTypeUtil.bormCard = 4;

// "火箭"
cardTypeUtil.rocketCard = 5;

// "三带一"
cardTypeUtil.threeTakeOneCard = 6;

// "三带二"
cardTypeUtil.threeTakeTwoCard = 7;

// "四带二单"
cardTypeUtil.fourTakeSingleCard = 8;

// "四带二双"
cardTypeUtil.fourTakeDoubleCard = 9;

// "顺子"
cardTypeUtil.sequenceCard = 10;

// "连对(双顺)"
cardTypeUtil.linkDoubleCard = 11;

// "飞机不带（三顺）"
cardTypeUtil.planeTakeNoneCard = 12;

// "飞机带单"
cardTypeUtil.planeTakeSingleCard = 13;

// "飞机带双"
cardTypeUtil.planeTakeDoubleCard = 14;

// "特殊牌形"
cardTypeUtil.spaceil = 15;

// "无效的牌形"
cardTypeUtil.undefindCard = 0;

// "4炸"(软炸)
cardTypeUtil.boomSoft = 104;

// "5炸"
cardTypeUtil.boomFive = 105;

// "6炸"
cardTypeUtil.boomSix = 106;

// "7炸"
cardTypeUtil.boomSeven = 107;

// "8炸"
cardTypeUtil.boomEight = 108;

// "小王的值"
cardTypeUtil.smallQueen = 78;

// "大王的值"
cardTypeUtil.bigQueen = 79;
...
cardTypeUtil.compare = function (selfCardIds, objCardIds) {
	if (selfCardIds == null || selfCardIds.length == 0) {
		console.log("自身牌参数为NULL值或为空数组");
		return false;
	};
	if (objCardIds == null || objCardIds.length == 0) {
		console.log("目标牌参数为NULL值或为空数组");
		return true;
	};
	var objCards = [];
...
cardTypeUtil.findShunZi = function (objcardValue, counts) {
	if (objcardValue.length == 0 || objcardValue == null) {
		console.log("参数有误00000000000000");
		return [];
	}
...
cardTypeUtil.findAllBigShunZi = function (selfCardValue, objcardShunZi) {
	if (objcardShunZi.length == 0 || objcardShunZi == null) {
		console.log("目标顺子数组有误00000000000000");
		return [];
	}
	if (selfCardValue.length == 0 || selfCardValue == null) {
		console.log("自己数组有误00000000000000");
		return [];
	}
