...
KBEngine.Entity = KBEngine.Class.extend(
    {
        ...
        //    }
           ...
           var method = KBEngine.moduledefs[this.className].base_methods[key];
           var methodID = method[0];
           var args = method[3];
        
           if (params.length != args.length) {
               KBEngine.ERROR_MSG("KBEngine.Entity::baseCall: args(" + (params.length) + "!= " + args.length + ") size is error!");
               return;
           }
...
KBEngine.DATATYPE_VECTOR = function (size) {
    ...
    this.parseDefaultValStr = function (v) {
        if (this.itemsize == 2) {
            ...
            return new KBEngine.Vector3(0.0, 0.0);
        }
        return new KBEngine.Vector2(0.0, 0.0);
    }
...
KBEngine.DATATYPE_ARRAY = function () {
    this.type = KBEngine.datatypes["UINT8"];
...
KBEngine.datatypes["ARRAY"] = new KBEngine.DATATYPE_ARRAY();
...
KBEngine.KBEngineApp = function (kbengineArgs) {
    ...
    this.onerror_before_onopen = function (evt) {
        KBEngine.ERROR_MSG('connect error 1:' + evt.data);
        KBEngine.Event.fire("onConnectStatus", false);
    }

    this.onerror_after_onopen = function (evt) {
        KBEngine.ERROR_MSG('connect error 2:' + evt.data);
    ...
    this.onclose = function () {
        ...
        KBEngine.Event.fire("onDisableConnect", args);
    }
    ...
    this.Client_onImportClientEntityDef = function (stream) {
        ...
        while (!stream.readEOF()) {
            ...
            var Class = KBEngine[scriptmethod_name];
            ...
            ;
            var defmethod = KBEngine[scriptmethod_name];
    ...
    this.Client_onLoginFailed = function (args) {
        ...
        var descr = "";
        if(24 < failedcode) {
            descr = KBEngine.utf8ArrayToString(KBEngine.app.serverdatas);
        }
        // }else {
        //     descr = KBEngine.app.serverErrs[failedcode].descr;
        // }
        // KBEngine.ERROR_MSG("KBEngineApp::Client_onLoginFailed: failedcode(" + KBEngine.app.serverErrs[failedcode].name + "), datas(" + descr + ")!");
        var datas = {"msg_type": "lobby", "err_type": "loginfailed", "errcode": failedcode, "errtxt": descr};
        KBEngine.Event.fire("onLoginFailed", datas);
    }
    ...
    this.Client_onLoginBaseappFailed = function (failedcode) {
        ...
        KBEngine.Event.fire("onLoginBaseappFailed", datas);
    }
    ...
    this.Client_onKicked = function (failedcode) {
        ...
        KBEngine.Event.fire("onKicked", datas);
    }
